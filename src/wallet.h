...
public:
    ...
    bool SendEDUToToken(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool SendTokenToEDU(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee=false);
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nEDU, int64_t& nToken) const
    {
        nEDU = 0;
        nToken = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
...
public:
    ...
    mutable int64_t nCredEDUCached;
    ...
    bool GetCredit(int64_t& nCredEDU, int64_t& nCredToken, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredEDU = nCredToken = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredEDUCached = 0;
            nCredTokenCached = 0;
            pwallet->GetCredit(*this, nCredEDUCached, nCredTokenCached);
            fCreditSplitCached = true;
        };
        
        nCredEDU = nCredEDUCached;
